---
title: 'Conversational Kludge: What Past Conceptions of Educational Twitter Hashtags
  Miss'
...
```{r, include=TRUE, echo=FALSE}
freq_replies <- edchat$reply_to_status_id %>%
        table %>%
        as.data.frame() %>%
        arrange(desc(Freq)) %>%
        rename(status_id = ".",
...
paste0(n_reply_tweeters, " (", p_reply_tweeters, 
       "% of all tweeters) replied to someone.")
```

```{r, include=TRUE, echo=FALSE}
freq_replies %>%
      summarize(mean = round(mean(reply_count), 2),
                sd = round(sd(reply_count), 2),
                median = median(reply_count),
                min = min(reply_count),
                max = max(reply_count)
                )
```
...
```{r, include=FALSE}
freq_replies_expanded <- edchat_expanded_og$reply_to_status_id %>% 
    table() %>% 
    as.data.frame() %>% 
    arrange(desc(Freq)) %>%
    rename(status_id = ".",
...
paste0(n_repliers_with_only, " (", p_repliers_with_only, "%) contributors to the #Edchat hashtag-thread mashup always included the #Edchat hashtag in their replies; ", n_repliers_both, " (", p_repliers_both, "%) contributors sometimes included the hashtag, sometimes not; and ", n_repliers_without_only, " (", p_repliers_without_only, "%) contributors never included the #Edchat hashtag in their replies.")
```
...
knitr::kable(table2, 
             align='l',
             col.names=c("With #Edchat", "During Sync %", "Self-reply %",
...
paste0("The network of the #Edchat hashtag-thread mashup has ", n_nodes, " nodes and ", n_edges, " edges,. The network has a diameter of ", diameter, ", a transitivity score of ", p_transitivity, "%, and a reciprocity score of ", p_reciprocity, "%. In addition, the mean degree is ", mean_degree, " (SD = ", sd_degree, ") with a median degree of ", median_degree, ". Finally the minimum degree is ", min_degree, " and the maximum degree is ", max_degree, ".")
```
