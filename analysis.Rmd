...
n_tweeters_no_RT <- edchat %>%
    ...
    unique() %>% 
    length()
paste("Number of unique tweets (no retweets):", n_tweets_no_RT); paste("Number of unique tweeters (no retweets):", n_tweeters_no_RT)
```
...
#length(unique(edchat_expanded$status_id))
```

**What proportion of replies contain the #Edchat hashtag?**

```{r, include=FALSE}
freq_replies_expanded <- edchat_expanded %>% 
    pull(reply_to_status_id) %>% 
    table() %>% 
    as.data.frame() %>% 
    rename(status_id = ".",
...
edchat_expanded <- edchat_expanded %>% 
    left_join(freq_replies, by='status_id') %>% 
    mutate(reply_count = ifelse(is.na(reply_count), 0, reply_count),
           in_edchat = ifelse(str_detect(text, "#[Ee][Dd][Cc][Hh][Aa][Tt]"), TRUE, FALSE),
...
paste("There are", n_all_replies, "tweets in the #Edchat reply network.", n_replies_with_edchat, "of these (", p_replies_with_edchat, "% ) contain the keyword text #edchat.")
```
...
1. Look at the difference in average number of *words* per tweet.

```{r, include=TRUE, echo=FALSE}
m_in_wordcount <- mean(replies_with_edchat$word_count)
m_out_wordcount <- mean(replies_without_edchat$word_count)
sig_wordcount <- t.test(x=replies_with_edchat$word_count, y=replies_without_edchat$word_count)
t_wordcount <- sig_wordcount$statistic %>% as.vector()
p_wordcount <- sig_wordcount$p.value %>% as.vector()
d_wordcount <- compute.es::tes(t_wordcount, 
                               n.1 = n_replies_with_edchat, 
                               n.2 = n_replies_without_edchat, verbose=FALSE)$d %>%
    abs()
row_word <- c(m_in_wordcount, m_out_wordcount, t_wordcount, p_wordcount, d_wordcount)
```

2. Look at the difference in average number of *characters* per tweet.

```{r, include=TRUE, echo=FALSE}
m_in_charcount <- mean(str_length(replies_with_edchat$text))
m_out_charcount <- mean(str_length(replies_without_edchat$text))
sig_charcount <- t.test(x=str_length(replies_with_edchat$text),
                        y=str_length(replies_without_edchat$text))
t_charcount <- sig_charcount$statistic %>% as.vector()
p_charcount <- sig_charcount$p.value %>% as.vector()
d_charcount <- compute.es::tes(t_charcount, 
                               n.1 = n_replies_with_edchat, 
                               n.2 = n_replies_without_edchat, verbose=FALSE)$d %>%
    abs()
row_character <- c(m_in_charcount, m_out_charcount, t_charcount, p_charcount, d_charcount)
```

3. Look at the difference in text-polarity *sentiment score* per tweet.
...
4. Look at the difference in average number of *hashtags* per tweet.
